...
test("term-llm bootstraps provider configuration before it becomes read-only", { skip: !process.env.TERM_LLM_BINARY }, (t) => {
  ...
  const env = { ...process.env, HOME: home, XDG_CONFIG_HOME: join(home, ".config"), ANTHROPIC_API_KEY: "bootstrap-test-secret" };
  const run = () => spawnSync(process.env.TERM_LLM_BINARY!, ["agents", "list"], { env, encoding: "utf8", timeout: 10000 });
  const first = run();
  ...
  for (const file of readdirSync(config)) {chmodSync(join(config, file), 0o444);}
