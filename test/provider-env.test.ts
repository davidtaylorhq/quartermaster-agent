...
test("the names are logged and the values are not", () => {
  ...
  assert.match(stdout, /given to the agent container: MISTRAL_API_KEY/);
  assert.doesNotMatch(
    stdout
      .split("\n")
...
for (const name of [
  ...
  test(`${name} reaches the agent unchanged`, () => {
    const { file, exported } = read(`${name}=secret-value\n`);
    assert.equal(file, `${name}=secret-value\n`);
    assert.doesNotMatch(exported, /MODEL_|secret-value/);
  });
}
