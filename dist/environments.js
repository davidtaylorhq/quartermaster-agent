...
// src/environments.ts
var KNOWN = ["name", "description", "image", "entrypoint", "cmd", "user", "mount", "setup"];
...
function main(source, dest) {
  ...
  const doc = load(readFileSync(source, "utf8"));
  ...
  list.forEach((entry, i) => {
    ...
    for (const field of ["image", "mount"]) {
      if (!String(env[field] ?? "")) fail(`${name} has no ${field}`);
    }
    for (const key of Object.keys(env)) {
      if (!KNOWN.includes(key)) fail(`${name} has an unknown setting "${key}"`);
    }
